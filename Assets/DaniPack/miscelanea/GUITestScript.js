...
//private var lifeScript : enemyLife;
//skin info
...
function DoMyWindow0 (windowID : int) 
{
	...
	// note: were passing the width of the window to the function
		...
		//recojo la vida
		var life = GameObject.Find("Enemy").GetComponent("enemyLife").life;
		
		GUILayout.BeginVertical();
		GUILayout.Space(8);
		GUILayout.Label("", "Divider");//-------------------------------- custom
        GUILayout.Label("VIDA ENEMIGO: ");
		GUILayout.Label(life.ToString(), "ShortLabel");//-------------------------------- custom
		GUILayout.Label("", "Divider");//-------------------------------- custom
		botonPegar = GUILayout.Button("PEGAR");
		botonVelocidad = GUILayout.Button("Reducir velocidad a 1");
		botonSaltar = GUILayout.Button("Sin Saltar");
		botonPausa = GUILayout.Button("CONTINUAR", "ShortButton");//-------------------------------- custom
		GUILayout.Label("", "Divider");//-------------------------------- custom
		...
		if(botonPegar){
			GameObject.Find("Enemy").SendMessage("restarVida", 50, SendMessageOptions.DontRequireReceiver);
		}
		
		if(botonVelocidad){
			transform.parent.gameObject.GetComponent("CharacterMotor").movement.maxForwardSpeed = 1;
			transform.parent.gameObject.GetComponent("CharacterMotor").movement.maxSidewaysSpeed = 1;
			transform.parent.gameObject.GetComponent("CharacterMotor").movement.maxBackwardsSpeed = 1;
		}
		
		if(botonSaltar){
			transform.parent.gameObject.GetComponent("CharacterMotor").jumping.enabled = false;
		}
...
function DoMyWindowStats (windowID : int) 
{
		// use the spike function to add the spikes
		...
		GUILayout.BeginVertical();
		GUILayout.Label ("", "Divider");//-------------------------------- custom
		GUILayout.Label ("", "PlainText");//------------------------------------ custom
...
if (doWindow1)
	windowRect1 = GUI.Window (1, windowRect1, DoMyWindowStats, "");
	//now adjust to the group. (0,0) is the topleft corner of the group.
	GUI.BeginGroup (Rect (0,0,100,100));
	// End the group we started above. This is very important to remember!
	GUI.EndGroup ();
